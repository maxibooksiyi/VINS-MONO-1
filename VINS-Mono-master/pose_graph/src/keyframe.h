...
class BriefExtractor //通过Brief模板文件，对图像的关键点计算Brief描述子
{
public:
...
public:
	...
	int index;  //关键帧index
	int local_index;
	Eigen::Vector3d vio_T_w_i; //位置向量 vio中第i帧在世界坐标系(第一个序列)的平移
	Eigen::Matrix3d vio_R_w_i;  //旋转矩阵  vio中第i帧在世界坐标系（第一个序列）的旋转
	Eigen::Vector3d T_w_i;
	Eigen::Matrix3d R_w_i;
	Eigen::Vector3d origin_vio_T;	// ？
	Eigen::Matrix3d origin_vio_R;   // ？
	cv::Mat image; //关键帧对应的图像
	cv::Mat thumbnail;
	vector<cv::Point3f> point_3d;  //关键帧能观测到的3d点
	vector<cv::Point2f> point_2d_uv;  //关键帧相机平面的2d点
	vector<cv::Point2f> point_2d_norm;  //关键帧上关键点归一化后的坐标
