...
//在Rviz可视化中显示闭环的边，就需要给Rviz中发送topic，
// add_loopedge函数当中创建了marker对象，并且设置了线条的画法 颜色 尺度和两个端点的坐标，
// 最后将marker对象塞入到m_markers当中,最终会在publish_by函数中遍历m_markers，
// 将组件markerarray数组，并将该topic发送出去
// 在Rviz当中订阅该topic(pose_graph)就可以进行闭环边的可视化，其他的可视化也是类似
/**
 * 闭环边添加函数
 * const Eigen::Vector3d& p0 当前关键帧的位置
 * const Eigen::Vector3d& p1 闭环帧的位置
*/
