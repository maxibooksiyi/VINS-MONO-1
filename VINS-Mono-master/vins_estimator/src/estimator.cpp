...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (!pre_integrations[frame_count])  //IMU测量值建立预积分类对象
    {
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    // 第二步 填充imageframe的容器以及更新临时预积分初始值 tmp_pre_integration = new IntegrationBase()
    ...
    if(ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            // 得到corres传入的是当前帧和其之前一帧的对应特征点对的3D坐标
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
                ESTIMATE_EXTRINSIC = 1; // 完成外参标定 后续需要优化
            }
        }
    }
    if (solver_flag == INITIAL) // 初始化 第一部分是纯视觉SfM优化滑窗内的位姿，然后在融合IMU信息，按照理论部分优化各个状态量
    {
        if (frame_count == WINDOW_SIZE)
        {
            bool result = false;
            //确保有足够的frame参与初始化，有外参，且当前帧时间戳大于初始化时间戳+0.1秒
            if( ESTIMATE_EXTRINSIC != 2 && (header.stamp.toSec() - initial_timestamp) > 0.1)  // 100HZ
            {
    ...
    {
        TicToc t_solve;
        solveOdometry();   //solver_flag = NON_LINEAR,进行非线性优化
        ROS_DEBUG("solver costs: %fms", t_solve.toc());

        if (failureDetection())  //边界判断:检测系统运行是否失败，若失败则重置估计器
        {
        ...
        TicToc t_margin;
        slideWindow();
        f_manager.removeFailures();  //去除估计失败的点并发布关键点位置
...
bool Estimator::initialStructure()
{
    ...
    // 1. 通过加速度标准差判断IMU是否有充分运动以初始化
    {
        ...
        // 判断，加速度标准差大于0.25则代表imu充分运动，足够初始化
        if(var < 0.25)
        {
    ...
    map<int, Vector3d> sfm_tracked_points;  //存储SFM重建出特征点的3D坐标
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        {
            imu_j++;
            Vector3d pts_j = it_per_frame.point;   // 3D特征点坐标 ? 为什么不是uv么？
            // 所有观测到该特征点的相机ID和图像坐标
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));
        }
        sfm_f.push_back(tmp_feature);
    }
    // 3 返回滑动窗口中第一个满足视差的帧，为l帧，以及R T,可以三角化
    Matrix3d relative_R;
    Vector3d relative_T;
    int l;
    ...
    GlobalSFM sfm;
    // 4. 对窗口中每个图像帧求解sfm问题
    // 得到所有图像帧到参考帧l的姿态四元数Q,平移向量T和特征点坐标sfm_tracked_points
    if(!sfm.construct(frame_count + 1, Q, T, l,
              relative_R, relative_T,
              sfm_f, sfm_tracked_points))
    {
        ROS_DEBUG("global SFM failed!");
        // 初始化后期的第一次slidingwindow() marg掉的是old帧  // 求解失败则边缘化最早一帧并滑动窗口
        marginalization_flag = MARGIN_OLD;
        return false;
    }

    //solve pnp for all frame
    // 5 对于所有的图像帧,包括不在滑动窗口中的,提供初始的R T估计
    map<double, ImageFrame>::iterator frame_it;
    map<int, Vector3d>::iterator it;
    frame_it = all_image_frame.begin();
    for (int i = 0; frame_it != all_image_frame.end( ); frame_it++)  // i:滑窗口
    {
        // provide initial guess
        cv::Mat r, rvec, t, D, tmp_r;
        //  a.边界判断：对于滑窗内的帧，把它们设为关键帧，并获得它们对应的IMU坐标系到l系的旋转平移
        if((frame_it->first) == Headers[i].stamp.toSec())  //对齐
        {
        ...
        Matrix3d R_inital = (Q[i].inverse()).toRotationMatrix();  //窗口外？
        ...
        //获取pnp需要用到的存储每个特征点三维点和图像坐标
        vector<cv::Point3f> pts_3_vector;   // 3D坐标
        vector<cv::Point2f> pts_2_vector;   // 2D坐标
        for (auto &id_pts : frame_it->second.points)   //frame_it帧下的所有特征点
        {
            int feature_id = id_pts.first;   // 特征点索引 map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>> > > points
            for (auto &i_p : id_pts.second)
            {
                it = sfm_tracked_points.find(feature_id);
                if(it != sfm_tracked_points.end())
                {
                    Vector3d world_pts = it->second;  // 3D坐标
                    cv::Point3f pts_3(world_pts(0), world_pts(1), world_pts(2));
                    pts_3_vector.push_back(pts_3);

                    Vector2d img_pts = i_p.second.head<2>(); // x.head<n>()函数:表示提取x前n个元素
        ...
        //保证特征点数大于5
        cv::Mat K = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, 1, 0, 0, 0, 1); // 内参数矩阵
        ...
        MatrixXd T_pnp;
        cv::cv2eigen(t, T_pnp);
        T_pnp = R_pnp * (-T_pnp);  //R_pnp:R^cl_ck ck到cl旋转矩阵/cl在ck下的旋转矩阵
        frame_it->second.R = R_pnp * RIC[0].transpose(); //RIC[0]:R^b_c
        frame_it->second.T = T_pnp;  //T^cl_ck
    }
...
bool Estimator::visualInitialAlign()
{
    ...
    VectorXd x;
    //solve scale
    // 计算陀螺仪偏置，尺度，重力加速度和速度  都是到l帧下的数据而不是c0
   ...
   // 重新计算所有f_manager的特征点深度
    ...
    ric[0] = RIC[0];
    f_manager.setRic(ric);
    // 计算特征点深度estimated_depth
    f_manager.triangulate(Ps, &(TIC_TMP[0]), &(RIC[0]));

    double s = (x.tail<1>())(0);
    //IMU的bias改变,重新计算滑窗内的预积分
    //solveGyroscopeBias()部分重复了，注释掉看看 现在又想了一下，感觉并不是重复了，因为6.4.1是更新完角速度bias后重新算了一下提升了精度，现在再算一下，又提升了一次精度
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
...
// 返回滑动窗口中第一个满足视差的帧，为l帧，以及R T,可以三角化
// relative_R 当前帧到第l帧之间的旋转矩阵R   relative_T 当前帧到第l帧之间的平移向量T
// l 从第一帧开始到滑动窗口中第一个满足视差足够的帧，这里的l帧之后作为参考帧做全局SFM用
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)  // 1. 首先corres数目足够
        {
            ...
            {
                Vector2d pts_0(corres[j].first(0), corres[j].first(1)); // x y
                Vector2d pts_1(corres[j].second(0), corres[j].second(1)); // x y
                double parallax = (pts_0 - pts_1).norm();  //二范数
                sum_parallax = sum_parallax + parallax;
            }
            //2. 判断每帧到窗口最后一帧对应特征点的平均视差是否大于30
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc t_tri;
        f_manager.triangulate(Ps, tic, ric);  //对新特征点进行三角化求深度(SVD分解)
...
void Estimator::vector2double()
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        para_Ex_Pose[i][0] = tic[i].x();  //平移 p^b_c
        para_Ex_Pose[i][1] = tic[i].y();
        para_Ex_Pose[i][2] = tic[i].z();
        Quaterniond q{ric[i]};     // 旋转 R^b_c/q^b_c
...
void Estimator::optimization()
{
    ...
    //loss_function = new ceres::HuberLoss(1.0);   //outliers little use   vins-fusion
    loss_function = new ceres::CauchyLoss(1.0); // 采用柯西核函数之后，使得当r_i过大时，其对应的目标函数不至于过大，从而起到抑制异常值的作用
    //  b. 添加各种待优化量X——位姿优化量
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if (!ESTIMATE_EXTRINSIC)   // 已经估计了外参数 //如果IMU-相机外参不需要标定
        {
    ...
    // e. vector2double() 给ParameterBlock赋值
    vector2double();
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            Vector3d pts_j = it_per_frame.point;  //！得到观测到该特征点的第二帧的归一化坐标
            ...
            f_m_cnt++;  //统计有多少个特征用于非线性优化
        }
    }

    ROS_DEBUG("visual measurement count: %d", f_m_cnt);
    ROS_DEBUG("prepare for ceres: %f", t_prepare.toc());
    // h. 添加各种残差——回环检测残差 计算滑动窗口中与每一个闭环关键帧的相对位姿，这个相对位置是为后面的图优化准备
    if(relocalization_info)
    {
    ...
    if (marginalization_flag == MARGIN_OLD)  //也是滑窗里的第一帧
    {
...

        {
            ...
            for (auto &it_per_id : f_manager.feature)
            {
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    Vector3d pts_j = it_per_frame.point;  // 除了第0帧全都加入
                    if (ESTIMATE_TD)
                    {
                        ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                          it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td,
        ...
        TicToc t_margin;
        marginalization_info->marginalize(); //多线程构造先验项舒尔补AX=b的结构，在X0处线性化计算Jacobian和残差
        ROS_DEBUG("marginalization %f ms", t_margin.toc());


        std::unordered_map<long, double *> addr_shift;  //散列表  将优化参数移动内存地址
...
void Estimator::slideWindow()
{
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        back_R0 = Rs[0];
        back_P0 = Ps[0];  //保存最老帧信息到back_R0,back_P0
        if (frame_count == WINDOW_SIZE)  //来了最新帧
        {
            for (int i = 0; i < WINDOW_SIZE; i++)
            {
                Rs[i].swap(Rs[i + 1]); //姿态

                std::swap(pre_integrations[i], pre_integrations[i + 1]);  //IMU预积分

                dt_buf[i].swap(dt_buf[i + 1]);
                linear_acceleration_buf[i].swap(linear_acceleration_buf[i + 1]); //线速度
