...
Eigen::Matrix2d ProjectionFactor::sqrt_info;    //像素噪声的标准差为1.5个像素
...
{
#ifdef UNIT_SPHERE_ERROR  //构造函数及切平面空间的基
    Eigen::Vector3d b1, b2;
    Eigen::Vector3d a = pts_j.normalized();
    Eigen::Vector3d tmp(0, 0, 1);
    if(a == tmp)
        tmp << 1, 0, 0;
    // a * (a.transpose() * tmp):tmp在a上的投影
    b1 = (tmp - a * (a.transpose() * tmp)).normalized();  //Gram-Schmidt正交化
    b2 = a.cross(b1);
...
#endif
        // 优化中使用的是马氏距离，需要用协方差矩阵Σ来做归一化,因此在残差和雅克比的计算中需要左乘Σ−1/2
