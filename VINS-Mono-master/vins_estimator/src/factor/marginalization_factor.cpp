...
{
    factors.emplace_back(residual_block_info); //在容器尾部添加一个元素，这个元素原地构造，不需要触发拷贝构造和转移构造
    ...
    for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++)
    {
        ...
        int size = parameter_block_sizes[i]; // 参数块大小
...
void MarginalizationInfo::preMarginalize()
{
    ...
    for (auto it : factors)
    {
        it->Evaluate();  // 分别计算所有状态变量构成的残差 和雅克比矩阵
...
{
    int pos = 0;
    for (auto &it : parameter_block_idx)   //待边缘化的变量
    {
        it.second = pos; //
        pos += localSize(parameter_block_size[it.first]);
    }

    m = pos;  //ｍ为需要marg掉的变量个数
    ...
    // 通过上面的操作就会将所有的优化变量进行一个伪排序，待marg的优化变量的idx为0，其他的和其所在的位置相关
    TicToc t_summing;
    ...
    */
    ...
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(Amm);   //求特征值
