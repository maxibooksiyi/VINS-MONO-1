...
class IntegrationBase
{
  public:
    ...
    // Eigen::Matrix<double, 15, 15>::Identity()单位矩阵
    // Eigen::Quaterniond::Identity() 单位四元数
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                    const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
           // 加速度计 陀螺仪 线性加速度计 线性陀螺仪
    ...
    void repropagate(const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
    {
        ...
        covariance.setZero();
        for (int i = 0; i < static_cast<int>(dt_buf.size()); i++) //得到前一帧b(k)和后一帧b(k+1)的信息
            propagate(dt_buf[i], acc_buf[i], gyr_buf[i]);
    }

    //// 两IMU测量时刻之间相邻时刻的pvq增量的中值积分离散形式
    // 时间间隔  上次测量加速度  上次测量角速度  本次测量加速度  本次测量角速度  相对初始参考系的位移  相对初始参考系的姿态
    // 相对初始参考系的速度  相对初始参考系的加速度偏差  相对初始参考系的角速度偏差  姿态变化量计算结果  位置变化量计算结果
    // 速度变化量计算结果  加速度偏差计算结果  角速度偏差计算结果   是否更新雅克比
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        if(update_jacobian)
        {
            ...
            F.block<3, 3>(0, 0) = Matrix3d::Identity();  //单位阵
            // f01
     ...
     //// 两帧之间相邻时刻的IMU的pvq增量传播
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1) // 时间间隔 线加速度 角速度
    {
        dt = _dt;
        acc_1 = _acc_1;
        gyr_1 = _gyr_1;
        ...
        midPointIntegration(_dt, acc_0, gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v,
                            ...
                            result_linearized_ba, result_linearized_bg, 1);   //covariance,jacobian

        //checkJacobian(_dt, acc_0, gyr_0, acc_1, gyr_1, delta_p, delta_q, delta_v,
        //                    linearized_ba, linearized_bg);
        // 变量迭代
        // 旋转增量:delta_q  速度增量:delta_v  位置增量:delta_p;
        delta_p = result_delta_p;
        delta_q = result_delta_q;
        delta_v = result_delta_v;
        // 加速度的偏差:linearized_ba  陀螺仪的偏差:linearized_bg;
    ...
    // 其作用是计算第i帧和第j帧之间的残差  sum_dt+=dt  dt:两帧之间的时间间隔
    // 第i帧位置  第i帧姿态  第i帧速度  第i帧加速度偏差  第i帧角速度偏差
    // 第j帧位置  第j帧姿态  第j帧速度  第j帧加速度偏差  第j帧角速度偏差
