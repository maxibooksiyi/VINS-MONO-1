...
// 利用五点法求解相机初始位姿
// Rotation:当前帧到参考帧的旋转矩阵  Translation:当前帧到参考帧的平移向量
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
        ...
        {
            ll.push_back(cv::Point2f(corres[i].first(0), corres[i].first(1)));  // x y
            rr.push_back(cv::Point2f(corres[i].second(0), corres[i].second(1)));
        }
        cv::Mat mask;
        // cv::findFundamentalMat 返回通过RANSAC算法求解两幅图像之间的本质矩阵E
        cv::Mat E = cv::findFundamentalMat(ll, rr, cv::FM_RANSAC, 0.3 / 460, 0.99, mask);
        // 相机内参矩阵
        cv::Mat cameraMatrix = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, 1, 0, 0, 0, 1);
        cv::Mat rot, trans;
        //  cv::recoverPose  通过本质矩阵得到R t，返回通过手性校验的内点个数
        ...
        Rotation = R.transpose();   // 第二帧坐标系到第一帧坐标系的旋转矩阵
        Translation = -R.transpose() * T;    // 第二帧坐标系到第一帧坐标系的平移矩阵
        if(inlier_cnt > 12)
            return true;
        else
            return false;
